...
### 7.8 rsync I/O errors during file push/copy

If you are pushing files to the drive from another system (e.g. TrueNAS, a laptop, etc.) and `rsync` reports `Input/output error (5)` on file metadata or directory creation, this indicates the drive has **actual media-level read/write errors** — not just a USB link instability. The `fsync` layer is failing, meaning the data cannot be reliably written to the physical platter/flash.
...
**What this means:** the drive's internal firmware is returning I/O errors when it tries to read or allocate blocks. This can happen alongside (or because of) the USB link resets we diagnosed earlier, but now the problem has moved deeper — the **drive media or its SATA bridge** can no longer guarantee data integrity.
...
1. **Remount read-only (safest).** The drive is telling us it can't guarantee writes. Remount it so nothing else writes to it:
   ```bash
   sudo umount /media/Media14Mir2
   sudo mount -o ro /dev/sdc1 /media/Media14Mir2
   ```
   Then copy any remaining data you need from this **read‑only** mount.

2. **Run `smartctl` — check the drive's self‑monitoring data.**
   ```bash
   sudo smartctl -a /dev/sdc
   ```
   Look for:
   ...
   - `UDMA_CRC_Error_Count` — DMA transfer errors (relevant if this were an HDD on ATA/IDE, less so on USB but still informative).
...
3. **Run `badblocks` (non‑destructive read‑only test).**
   ...
   ```
   This scans every block for read errors. On a 14 TB drive this can take many hours. If errors are found, the drive is definitely failing.

4. **If the drive is an external USB‑SATA enclosure with a removable drive,** you can:
   ...
   - Run `smartctl` and `badblocks` on the PC — this bypasses the potentially‑faulty USB‑SATA bridge entirely.
...
5. **If the drive is still under warranty,** initiate a return/replacement with the manufacturer (TrueNAS/ZFS may also be able to `zpool replace` if you have mirror redundancy).
...
| `rsync` reports `Input/output error (5)` on file stat/mkdir | See [§7.8 rsync I/O errors during file push/copy](#78-rsync-io-errors-during-file-push-copy). The drive has media‑level write failures; remount read‑only immediately, then run `smartctl` / `badblocks` to assess drive health. |
